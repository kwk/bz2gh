...
def ensure_enough_requests(ask_for_num_requests=1):
    ...
    global remaining_requests_last_refreshed
    if remaining_requests <= ask_for_num_requests or (time.time() - remaining_requests_last_refreshed) > 60:
        while True:
            remaining_requests = gh.get_rate_limit().core.remaining
            remaining_requests_last_refreshed = time.time()
            logger.debug("Refreshed remaining github request: %d remaining", remaining_requests)
            if remaining_requests <= ask_for_num_requests:
                seconds_to_wait = 300
                logger.warning("Number of remaining Github requests is too low (%d). Waiting %fs until we can continue." % (remaining_requests, seconds_to_wait))
